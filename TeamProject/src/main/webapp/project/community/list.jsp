...
<body bgcolor="<%=bodyback_c%>">
 ...
 %> <img
					src="img/hot.gif" border="0" height="16"> <%
 }
 %>
				</td>
		...
		if (count > 0) {
			...
			if (startPage > pageBlock) {
				...
				if (searchText == null) {
		%>
		<a href="list.jsp?pageNum=1">[<<]</a> <a
			href="list.jsp?pageNum=<%=startPage - pageBlock%>">[이전]</a> <a
			href="list.jsp?pageNum=<%=(Integer.parseInt(pageNum) - 1)%>">[<]</a>
		<%
		} else {
		%>
		<a
		...
		<a href="list.jsp?pageNum=<%=startPage + pageBlock%>">[다음]</a> <a
			href="list.jsp?pageNum=<%=pageCount%>">[>>]</a>
		<%
		} else {
		%>
		<a
