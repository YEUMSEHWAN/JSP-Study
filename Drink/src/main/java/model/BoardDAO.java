...
public class BoardDAO {
	...
	public void insertBoard(BoardBean bean) {
		...
		try {
			...
			String sql = "insert into board(writer, email, subject, password, reg_date, ref, re_step, re_level, readcount, content) values(?,?,?,?,sysdate(),?,?,?,0,?)";
...
//			SELECT * FROM( SELECT (@ROWNUM := @ROWNUM + 1) AS RN, A.* FROM ( SELECT * FROM board)A, (SELECT @ROWNUM := 0) AS X LIMIT ?)B WHERE RN >= ?
					/*
	...
	public void reWriteBoard(BoardBean bean) {
		...
		try {
			...
			pstmt.setString(3, bean.getSubject());
			pstmt.setString(4, bean.getPassword());
			pstmt.setInt(5, ref);//무모의 ref값을 넣어줌
